...
import com.world.wen.util.ILog;
import com.world.wen.util.MD5Util;
import com.world.wen.util.OffLineDataUtil;
import com.world.wen.view.fragment.BaseFragment;
	...
	public void postString(final String url, final Map<String, String> param, final Map<String, String> mheaders, final String tag,
		...
		StringRequest stringRequest = new StringRequest(Method.POST, url, new Response.Listener<String>() {
			...
			public void onResponse(String response) {
				Log.v(TAG, "[服务器返回] [" + tag + "]=" + response);
				listener.onResponseOK(response, tag);
				Wen9Application.isInOfflineState=false;
				if (isCache) {// 如果需要缓存
					JSONObject json;
					try {
			...
			public void onErrorResponse(VolleyError error) {
				printStackTrace(tag, error);
				if (isCache) {
					if(Wen9Application.isInOfflineState==false)
					{
						Wen9Application.isInOfflineState=true;
			...
			protected Response<String> parseNetworkResponse(NetworkResponse response) {
				...
				if (response.headers.containsKey("Set-Cookie")) {
					...
					if (Wen9Application.SESSION_ID == null||Wen9Application.SESSION_ID.length()==0)
						Wen9Application.SESSION_ID = temp;
				}
